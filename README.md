...
- **AI 응답을 기다리는 40초 동안 DB 커넥션을 붙잡고 있던 것** — `@Transactional` 하나가 외부 LLM 호출까지 감싸 커넥션이 60초 넘게 잡히고, 실패 시 가입까지 롤백돼 22명이 계정째 사라짐. `performance_schema` 폴링으로 점유 22.5초→0초 실측. 테스트 0개인 코드라 **특성화 테스트 52개를 먼저 원본에서 green으로 만든 뒤** 리팩토링

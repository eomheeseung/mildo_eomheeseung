...
- **주 CTA가 통째로 사라진 채 운영에 나감** — 가로 2버튼용 `flex: 1`(=`flexBasis: 0`)을 세로 배치에 재사용해 높이가 0. 에러 없음 + 웹은 정상이라 늦게 발견 → `flex: 0`으로 같은 키를 덮어 해결
- **일일 보상이 하루 통째로 누락될 수 있던 구조** — 클라이언트 시계로 "오늘 했나"를 판정. `granted:false`가 두 상태를 뜻해 구분 불가 → 서버에 판정 날짜 필드를 요청해 기준을 서버로 이관
